...
try {
    ...
    $stmt = $connexio->prepare("UPDATE products SET name =:name, description = :description, price= :price, q_sold= :q_sold WHERE ID =:id");
    ...
    $stmt->execute(array(':name' => $nom, ':description' => $desc, ':price' => $preu, ':q_sold' => $q_sold, ':id' => $id));
